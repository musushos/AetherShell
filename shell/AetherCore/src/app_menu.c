...
void app_menu_set_relative_to(GtkWidget *menu, GtkWidget *relative_to) {
    ...
    AetherCore_window_backend_align_popup(GTK_WINDOW(menu), relative_to, 220);
}
...
GtkWidget* init_app_menu(void) {
    ...
    AetherCore_window_backend_set_anchor(GTK_WINDOW(popover), GTK_LAYER_SHELL_EDGE_LEFT, TRUE);
    AetherCore_window_backend_set_margin(GTK_WINDOW(popover), GTK_LAYER_SHELL_EDGE_TOP, AetherCore_offset);
    AetherCore_window_backend_set_margin(GTK_WINDOW(popover), GTK_LAYER_SHELL_EDGE_LEFT, 0);
    ensure_rgba_visual(popover);
    
    AetherCore_surface = gtk_event_box_new();
    gtk_widget_set_name(AetherCore_surface, "app-menu-box");
    gtk_event_box_set_visible_window(GTK_EVENT_BOX(AetherCore_surface), TRUE);
    ensure_rgba_visual(AetherCore_surface);
    g_signal_connect(AetherCore_surface, "draw", G_CALLBACK(draw_app_menu_background), NULL);
