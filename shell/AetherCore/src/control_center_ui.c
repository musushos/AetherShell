...
void control_center_set_relative_to(GtkWidget *control_center, GtkWidget *relative_to) {
    ...
    AetherCore_window_backend_align_popup(GTK_WINDOW(control_center), relative_to, 300);
}
...
static GtkWidget* create_controls_page() {
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(icon_disp), "slider-AetherCore-icon");
    GtkWidget *lbl_disp = gtk_label_new("Display");
    gtk_style_context_add_class(gtk_widget_get_style_context(lbl_disp), "slider-AetherCore-title");
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(icon_sun), "slider-AetherCore-icon");
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(icon_snd), "slider-AetherCore-icon");
    GtkWidget *lbl_snd = gtk_label_new("Sound");
    gtk_style_context_add_class(gtk_widget_get_style_context(lbl_snd), "slider-AetherCore-title");
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(icon_note), "slider-AetherCore-icon");
...
GtkWidget* init_control_center(void) {
    ...
    GtkWidget *AetherCore_surface;
    char *css_path = AetherCore_resource_path("style.css");
    if (g_file_test(css_path, G_FILE_TEST_EXISTS)) {
        gtk_css_provider_load_from_path(provider, css_path, NULL);
    } else {
        g_warning("Could not find AetherCore style.css at %s", css_path);
    }
    g_free(css_path);
    ...
    AetherCore_surface = gtk_event_box_new();
    ...
    g_signal_connect(AetherCore_surface, "draw", G_CALLBACK(draw_control_center_background), NULL);
