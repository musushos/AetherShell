/*
 * AetherCore.c — AetherShell AetherCore
 *
 * Bootstrap:
 *   1. gtk_init
 ...
 *   7. Build the pill-based layout from AetherCore.json
 *   8. gtk_main loop
 */
...
/* Default AetherCore height (JSON can override this at startup) */
#define DEFAULT_AetherCore_HEIGHT  36

/* External plugin directory — scanned for *.so at startup */
#define EXTERNAL_PLUGIN_DIR   "config/aether/plugins"

/* Installed system-wide AetherCore layout JSON */
#define DEFAULT_LAYOUT_JSON       "/usr/local/share/AetherCore/AetherCore.json"

/* Relative path from binary dir used during development (./config/AetherCore.json) */
#define DEV_LAYOUT_JSON_RELPATH   "config/AetherCore.json"

/* ── Default AetherCore.json written when no config is found ───────────────── */
static const char *AetherCore_DEFAULT_JSON =
    "{\n"
    "  \"_comment\": \"AetherShell AetherCore — auto-generated default layout.\",\n"
    "  \"AetherCore\": {\n"
    "    \"spacing\" : 8,\n"
...
static int      g_AetherCore_height      = DEFAULT_AetherCore_HEIGHT;

/* ── Layout live-reload state ───────────────────────────────────────────────── */

static char          *g_layout_json_path = NULL; /* monitored file path        */
static GtkWidget     *g_AetherCore_bar        = NULL; /* outer bar — always alive   */
static GtkWidget     *g_content          = NULL; /* current layout content     */
static GFileMonitor  *g_layout_monitor   = NULL; /* GFileMonitor for AetherCore.json */
...
static gboolean restart_AetherCore(void)
{
    if (!g_executable_path || g_executable_path[0] == '\0') {
        g_warning("[AetherCore] Cannot restart: executable path unavailable");
        return FALSE;
    }
    GError *err  = NULL;
    ...
    if (!g_spawn_async(NULL, argv, NULL, G_SPAWN_SEARCH_PATH,
                       NULL, NULL, NULL, &err)) {
        g_warning("[AetherCore] Restart failed: %s", err ? err->message : "?");
...
static void on_AetherCore_window_destroy(GtkWidget *w, gpointer d)
{
    (void)w; (void)d;
...
static void on_screen_changed(GdkScreen *screen, gpointer user_data)
{
    ...
    if (get_widget_monitor_geometry(window, &geom)) {
        gtk_widget_set_size_request(window, geom.width, g_AetherCore_height);
        gtk_window_resize(GTK_WINDOW(window), geom.width, g_AetherCore_height);
    }
}
...
static gboolean do_reload_layout(gpointer data)
{
    ...
    if (!g_AetherCore_bar || !g_layout_json_path) return G_SOURCE_REMOVE;

    g_debug("[AetherCore] Live-reloading layout from %s", g_layout_json_path);
    ...
    if (!new_content) {
        g_warning("[AetherCore] Layout reload: builder returned NULL");
        plugin_engine_release_saved_refs();
        return G_SOURCE_REMOVE;
    }

    g_content = new_content;
    gtk_box_pack_start(GTK_BOX(g_AetherCore_bar), g_content, TRUE, TRUE, 0);
    gtk_widget_show_all(g_AetherCore_bar);
...
static void start_layout_monitor(const char *path)
{
    ...
    if (!g_layout_monitor) {
        g_warning("[AetherCore] Cannot monitor layout file: %s",
                  err ? err->message : "?");
        if (err) g_error_free(err);
        return;
    }
    ...
    g_debug("[AetherCore] Monitoring layout: %s", path);
}

/* ── Input pass-through (click-through on empty areas) ─────────────────────── */

/*
 * Recursively collect the screen rectangles of all leaf widgets that are
 * visible and have an allocated size > 1×1.  These are the only areas that
 * should receive pointer events.  Empty transparent space in the fullscreen
 * overlay is excluded — clicks fall through to the window underneath.
 */
static void collect_input_rects(GtkWidget       *widget,
                                cairo_region_t  *region)
{
    ...
    if (GTK_IS_CONTAINER(widget)) {
        ...
        for (GList *l = children; l; l = l->next)
            collect_input_rects(GTK_WIDGET(l->data), region);
        g_list_free(children);
        return;
    }

    /* Leaf widget — add its allocation to the region */
    GtkAllocation alloc;
...
static void on_AetherCore_size_allocate(GtkWidget     *window,
                                   GtkAllocation *alloc,
                                   gpointer       user_data)
{
    (void)alloc;
    ...
    /* Build an input region that only covers actual pill widgets */
    cairo_region_t *region = cairo_region_create();
    collect_input_rects(window, region);
...
static GtkWidget *create_AetherCore_window(GdkScreen *screen)
{
    ...
    /* Layer-shell: anchor ALL four edges → fullscreen overlay */
    AetherCore_window_backend_init_AetherCore(GTK_WINDOW(window), "con-AetherCore");
    AetherCore_window_backend_set_anchor(GTK_WINDOW(window), GTK_LAYER_SHELL_EDGE_TOP,    TRUE);
    AetherCore_window_backend_set_anchor(GTK_WINDOW(window), GTK_LAYER_SHELL_EDGE_BOTTOM, TRUE);
    AetherCore_window_backend_set_anchor(GTK_WINDOW(window), GTK_LAYER_SHELL_EDGE_LEFT,   TRUE);
    AetherCore_window_backend_set_anchor(GTK_WINDOW(window), GTK_LAYER_SHELL_EDGE_RIGHT,  TRUE);

    /* Zero margins — occupy the entire screen geometry */
    AetherCore_window_backend_set_margin(GTK_WINDOW(window), GTK_LAYER_SHELL_EDGE_TOP,    0);
    AetherCore_window_backend_set_margin(GTK_WINDOW(window), GTK_LAYER_SHELL_EDGE_BOTTOM, 0);
    AetherCore_window_backend_set_margin(GTK_WINDOW(window), GTK_LAYER_SHELL_EDGE_LEFT,   0);
    AetherCore_window_backend_set_margin(GTK_WINDOW(window), GTK_LAYER_SHELL_EDGE_RIGHT,  0);

    /* NO exclusive zone — windows can tile/maximise behind the overlay */
    ...
    /* Wire up input pass-through: recompute input region after every layout */
    g_signal_connect(window, "size-allocate",
                     G_CALLBACK(on_AetherCore_size_allocate), NULL);

    /* Background container */
    GtkWidget *AetherCore_bar = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
    gtk_widget_set_name(AetherCore_bar, "AetherCore-bar");
    gtk_container_add(GTK_CONTAINER(window), AetherCore_bar);

    return window;
}

/* ── Resolve (or generate) AetherCore.json path ────────────────────────────────── */

static char *resolve_layout_json_path(void)
{
    /* 1. User override: ~/.config/aether/AetherCore.json */
    char *user_path = g_build_filename(g_get_user_config_dir(),
                                       "aether", "AetherCore.json", NULL);
    if (g_file_test(user_path, G_FILE_TEST_EXISTS)) {
        g_debug("[AetherCore] Using user layout: %s", user_path);
        return user_path;
    }
    g_free(user_path);
    ...
    if (g_file_test(DEFAULT_LAYOUT_JSON, G_FILE_TEST_EXISTS)) {
        g_debug("[AetherCore] Using system layout: %s", DEFAULT_LAYOUT_JSON);
        return g_strdup(DEFAULT_LAYOUT_JSON);
    }

    /* 3. Development path — next to the binary (./config/AetherCore.json) */
    if (g_executable_path) {
        ...
        if (g_file_test(dev_path, G_FILE_TEST_EXISTS)) {
            g_debug("[AetherCore] Using dev layout: %s", dev_path);
            return dev_path;
        }
        g_free(dev_path);
    }

    /* 4. Nothing found — auto-generate at ~/.config/aether/AetherCore.json */
    ...
    GError *err = NULL;
    if (g_file_set_contents(gen_path, AetherCore_DEFAULT_JSON, -1, &err)) {
        g_message("[AetherCore] Generated default AetherCore.json at: %s", gen_path);
        return gen_path;
    }

    g_warning("[AetherCore] Could not generate AetherCore.json: %s",
...
int main(int argc, char *argv[])
{
    ...
    AetherCore_compositor_backend_init();
    ...
    /* ── CSS ──────────────────────────────────────────────────────────────── */
    char *base_css = AetherCore_resource_path_in("", "style.css");
    AetherCore_css_provider_init(base_css);
    g_free(base_css);
    ...
    GtkWidget  *window = create_AetherCore_window(screen);

    g_signal_connect(window, "destroy", G_CALLBACK(on_AetherCore_window_destroy), NULL);
    ...
    /* Keep a reference to AetherCore_bar for live-reload */
    g_AetherCore_bar = gtk_bin_get_child(GTK_BIN(window));
    if (g_AetherCore_bar && g_content)
        gtk_box_pack_start(GTK_BOX(g_AetherCore_bar), g_content, TRUE, TRUE, 0);

    /* ── Start monitoring AetherCore.json for live layout reload ──────────────── */
