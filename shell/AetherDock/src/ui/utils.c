...
const gchar *dock_get_resource_base_dir(void) {
    if (dock_resource_base_dir == NULL) {
        Dl_info dl_info;
        if (dladdr((void *)dock_get_resource_base_dir, &dl_info) != 0 && dl_info.dli_fname != NULL) {
            gchar *real_path = realpath(dl_info.dli_fname, NULL);
            if (real_path) {
                dock_resource_base_dir = g_path_get_dirname(real_path);
                free(real_path);
            } else {
                dock_resource_base_dir = g_path_get_dirname(dl_info.dli_fname);
            }
        } else {
