...
static AetherPanelContext *plugin_ctx = NULL;
...
static GtkWidget *create_context_menu_item(const gchar *label, WindowGroup *group, void (*action)(GtkWidget *, gpointer)) {
    ...
    gtk_widget_set_name(button, "AetherDock-context-menu-item");
...
static GtkWidget *create_context_menu_separator(void) {
    ...
    gtk_widget_set_name(separator, "AetherDock-context-menu-separator");
    return separator;
}
...
static GtkWidget *create_wayland_menu_item(const gchar *label, WindowGroup *group, void (*action)(GtkWidget *, gpointer)) {
    ...
    gtk_widget_set_name(item, "dock-AetherDock-context-menu-item");
    gtk_style_context_add_class(context, "dock-AetherDock-context-menu-item");
    action_data->group = group;
    action_data->action = action;
...
static GtkWidget *create_launch_ring(void) {
    ...
    gtk_widget_set_name(ring, "AetherDock-launch-ring");
...
static void attach_launch_ring(GtkWidget *button) {
    ...
    GtkWidget *ring = g_object_get_data(G_OBJECT(button), "AetherDock-launch-ring");
    ...
    g_object_set_data(G_OBJECT(button), "AetherDock-launch-ring", ring);
...
static GtkWidget* create_widget(AetherPanelContext *ctx) {
    ...
    GError *error = NULL;
    gchar *css_path = dock_build_resource_path("AetherDock.css");
    ...
    gtk_widget_set_name(box, "AetherDock-dock-box"); /* ID for CSS */
    ...
    create_trash_button(box);
    create_drives_area(box);
    system_separator = gtk_separator_new(GTK_ORIENTATION_VERTICAL);
    gtk_widget_set_name(system_separator, "AetherDock-system-separator");
...
AetherPanelPluginAPIv3* aether_panel_plugin_init_v3(void) {
    static AetherPanelPluginAPIv3 api = {0};
    api.api_version = AETHER_PANEL_PLUGIN_API_VERSION;
    api.struct_size = sizeof(AetherPanelPluginAPIv3);
    api.name = "AetherDock";
    api.description = "Mac-like dock for Wayland";
    api.author = "Aether";
    api.version = "1.0.0";
    api.icon_name = "preferences-desktop-display";
    api.zone = AETHER_PLUGIN_ZONE_CENTER;
    api.singleton = TRUE;
    api.visuals.bg_type = AETHER_PLUGIN_BG_TRANSPARENT;
    api.create_widget = create_widget;
    api.destroy_widget = destroy_widget;
    api.get_theme = get_theme;
    return &api;
}
...
void update_window_list() {
    ...
    if (TRUE) {
        ...
        while (g_hash_table_iter_next(&hash_iter, &key, &value)) {
            ...
            gtk_widget_set_name(dot, "AetherDock-indicator-dot");
            ...
            if (window_count > 0) {
                gtk_style_context_add_class(context, "AetherDock-running-app");
            } else {
                gtk_style_context_add_class(context, "AetherDock-pinned-app");
            }
        ...
        gtk_widget_show_all(box);
        if (plugin_ctx && plugin_ctx->request_panel_resize) {
            plugin_ctx->request_panel_resize(0);
        }
    }
}
...
void load_pinned_apps() {
    ...
    gchar *config_file = g_build_filename(config_dir, "AetherDock-pinned-apps", NULL);
...
void create_context_menu(WindowGroup *group, GdkEventButton *event) {
    ...
    if (TRUE) {
        ...
        gtk_widget_set_name(menu, "AetherDock-dock-context-menu");
        gtk_widget_set_app_paintable(menu, TRUE);
        gtk_container_set_border_width(GTK_CONTAINER(menu), 0);
        gtk_menu_set_reserve_toggle_size(GTK_MENU(menu), FALSE);
        gtk_style_context_add_class(context, "AetherDock-dock-context-menu");
