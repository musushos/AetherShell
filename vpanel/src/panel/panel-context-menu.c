...
static void on_menu_preferences(GtkMenuItem *item, gpointer data)
{
    (void)item;
    (void)data;
    char *settings_bin = g_find_program_in_path("vpanel-settings");
    if (settings_bin) {
        g_spawn_command_line_async(settings_bin, NULL);
        g_free(settings_bin);
        return;
    }
    g_spawn_command_line_async("./vpanel-settings", NULL);
}
...
static void on_menu_restart(GtkMenuItem *item, gpointer data)
{
    (void)item;
    (void)data;
    char *panel_bin = g_find_program_in_path("vpanel");
    const char *panel_cmd = panel_bin ? panel_bin : "./vpanel";
    char *cmd = g_strdup_printf("bash -c 'pidof vpanel | xargs -r kill; nohup %s >/dev/null 2>&1 &'", panel_cmd);
